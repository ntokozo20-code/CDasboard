...
export default function LiveMarketItem({ name, symbol, change, price }) {
  const getIcon = () => {
    ...
    if (name.includes("Ethereum"))
      return <SiEthereum className="text-indigo-500" />;
    if (name.includes("Litecoin"))
      return <SiLitecoin className="text-blue-500" />;
    if (name.includes("Cardano"))
      return <SiCardano className="text-green-500" />;
  };
  ...
  return (
    <div className="flex justify-between items-center mb-3">
      ...
      <div className="text-right">
        <p
          className={`${change.includes("-") ? "text-red-500" : "text-green-500"} font-semibold`}
        >
          {change}
        </p>
